...
<script>
import { url_23, url,url_40} from "../../serviceAPI.config.js";
...
export default {
  ...
  created() {
    ...
    if (this.$route.params.id) {
      ...
      localStorage.setItem("recomId", this.$route.params.id);
    } else {
      this.id = localStorage["recomId"];
    }
    this.axios({
      method: "get",
      params: {
    ...
    payStyle() {
      for (let item of this.personNum) {
       ...
       if(!this.exist){
        ...
        console.log(this.dateTime[this.dateNum - 1].startTime)
       this.axios({
        method:"post",
        data:{
